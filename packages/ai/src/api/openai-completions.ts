...
export const stream: StreamFunction<"openai-completions", OpenAICompletionsOptions> = (
	...
	(async () => {
		...
		try {
			...
			// Redacted stream trace for truncation diagnostics: only event kinds,
			// ids and lengths are recorded — never content.
			const streamTrace: {
				eventTypes: Set<string>;
				blockLengths: { type: string; count: number; chars: number }[];
				blocksSeen: Map<string, number>;
				charsSeen: Map<string, number>;
			} = {
				eventTypes: new Set(),
				blockLengths: [],
				blocksSeen: new Map(),
			...
			const recordBlockActivity = (type: string, deltaLength: number): void => {
				streamTrace.blocksSeen.set(type, (streamTrace.blocksSeen.get(type) ?? 0) + 1);
				streamTrace.charsSeen.set(type, (streamTrace.charsSeen.get(type) ?? 0) + deltaLength);
			};
			const buildStreamTraceDiagnostic = (): AssistantMessageDiagnostic => {
				streamTrace.blockLengths = [...streamTrace.blocksSeen.entries()].map(([type, count]) => ({
					type,
					count,
					chars: streamTrace.charsSeen.get(type) ?? 0,
				}));
				return {
					...
					details: {
						...
						sseEventTypes: [...streamTrace.eventTypes],
						receivedDoneSentinel: streamTrace.eventTypes.has("done"),
						receivedFinishReason: hasFinishReason,
						blockTypes: streamTrace.blockLengths,
					},
				};
			};
			...
			for await (const chunk of openaiStream) {
				...
				streamTrace.eventTypes.add("chunk");
				if ((chunk as { _done?: boolean })._done) {
					streamTrace.eventTypes.add("done");
				}
				...
				if (choice.delta) {
					if (
						...
						recordBlockActivity("text", choice.delta.content.length);
						stream.push({
							type: "text_delta",
							contentIndex: getContentIndex(block),
					...
					if (foundReasoningField) {
						...
						if (typeof delta === "string" && delta.length > 0) {
							...
							recordBlockActivity("thinking", delta.length);
							stream.push({
								type: "thinking_delta",
								contentIndex: getContentIndex(block),
								delta,
					...
					if (choice?.delta?.tool_calls) {
						for (const toolCall of choice.delta.tool_calls as StreamingToolCallDelta[]) {
							...
							if (toolCall.function?.arguments) {
								...
								block.arguments = parseStreamingJson(block.partialArgs);
								recordBlockActivity("toolCall", delta.length);
			...
			if ((compat.supportsFinishReason && !hasFinishReason) || output.stopReason === "pending") {
				const trace = buildStreamTraceDiagnostic();
				output.diagnostics = [...(output.diagnostics ?? []), trace];
				const eventTypes = Array.isArray(trace.details?.sseEventTypes) ? trace.details.sseEventTypes : [];
				throw new Error(
					`Stream ended without finish_reason (received ${eventTypes.length} event types, finish_reason=${hasFinishReason})`,
				);
			}
