...
describe("openai-completions tool_choice", () => {
	...
	it("errors when a stream ends after only null finish_reason chunks", async () => {
		...
		expect(response.stopReason).toBe("error");
		expect(response.errorMessage).toContain("Stream ended without finish_reason");
		expect(response.errorMessage).toContain("finish_reason=false");
		// Redacted truncation diagnostics: event kinds and lengths, no content.
		expect(response.diagnostics).toMatchObject([
			{
				...
				details: {
					...
					sseEventTypes: ["chunk"],
					receivedDoneSentinel: false,
					receivedFinishReason: false,
					blockTypes: [{ type: "text", count: 2, chars: 28 }],
				},
			},
		]);
		expect(response.diagnostics?.[0].timestamp).toEqual(expect.any(Number));
		expect(JSON.stringify(response.diagnostics)).not.toContain("partial answer");
	});
