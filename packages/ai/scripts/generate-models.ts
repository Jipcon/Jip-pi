...
function applyThinkingLevelMetadata(model: Model<any>): void {
	...
	if (
		(model.api === "openai-completions" || (model.api === "openai-responses" && model.provider === "deepseek")) &&
	...
	if (model.provider === "opencode-go" && model.id === "kimi-k3") {
		// Kimi K3 officially exposes low/high/max reasoning tiers; models.dev
		// upstream data only reports max.
		mergeThinkingLevelMap(model, { low: "low", high: "high" });
	}
