...
const ThemeJsonSchema = Type.Object({
	...
	colors: Type.Object({
		...
		// Generic shell mode; falls back to bashMode when absent.
		shellMode: Type.Optional(ColorValueSchema),
	}),
...
): ThemeJson["colors"] & { thinkingMax: ColorValue; scrollbarThumb: ColorValue; shellMode: ColorValue } {
	return {
		...colors,
		thinkingMax: colors.thinkingMax ?? colors.thinkingXhigh,
		scrollbarThumb: colors.scrollbarThumb ?? colors.selectedBg,
		// shellMode is the canonical interactive-shell color; existing themes
		// only define bashMode, so fall back for compatibility.
		shellMode: colors.shellMode ?? colors.bashMode,
	};
}
...
export class Theme {
	...
	/**
	 * Border color for interactive ! shell mode.
	 * Resolves to shellMode, falling back to bashMode for legacy themes.
	 */
	getShellModeBorderColor(): (str: string) => string {
		return (str: string) => this.fg("shellMode", str);
	}
}
