...
export interface CreateCodingAgentHarnessOptions extends Omit<AgentHarnessOptions, "toolContext" | "tools"> {
	...
	sessionFile?: string;
	/**
	 * Model-visible working directory for the system prompt. When set, the
	 * prompt uses this logical root while tools keep executing against
	 * `env.cwd`; hidden workspace workers must pass this so no sibling
	 * physical path ever reaches the model.
	 */
	logicalCwd?: string;
