...
import { getDefaultCommandToolName } from "../core/tools/index.ts";
	...
	const extensionFlagsText =
		extensionFlags && extensionFlags.length > 0
			? `\n${chalk.bold("Extension CLI Flags:")}\n${extensionFlags
					...
					.join("\n")}\n`
			: "";
	console.log(`${chalk.bold(APP_NAME)} - AI coding assistant with built-in read, bash, pwsh, edit, write tools (platform default: read, ${getDefaultCommandToolName()}, edit, write)
...
${chalk.bold("Built-in Tool Names:")}
  read   - Read file contents (default)
  bash   - Execute bash commands (default on Linux/macOS)
  pwsh   - Execute PowerShell commands (default on Windows)
  edit   - Edit files with find/replace (default)
  write  - Write files (creates/overwrites) (default)
  grep   - Search file contents (read-only, off by default)
  find   - Find files by glob pattern (read-only, off by default)
  ls     - List directory contents (read-only, off by default)

Default tools on this platform: read, ${getDefaultCommandToolName()}, edit, write.
`);
}
