...
export class ExtensionRunner {
	...
	/**
	 * Emit the canonical user_shell interception event.
	 *
	 * Unlike the legacy user_bash path, an empty result object is treated as
	 * unhandled: only a result with `result` or `operations` set short-circuits.
	 * This keeps the canonical-first, legacy-fallback dispatch deterministic.
	 */
	async emitUserShell(event: UserShellEvent): Promise<UserShellEventResult | undefined> {
		const ctx = this.createContext();

		for (const ext of this.extensions) {
			const handlers = ext.handlers.get("user_shell");
			if (!handlers || handlers.length === 0) continue;

			for (const handler of handlers) {
				try {
					const handlerResult = (await handler(event, ctx)) as UserShellEventResult | undefined;
					if (handlerResult?.result !== undefined || handlerResult?.operations !== undefined) {
