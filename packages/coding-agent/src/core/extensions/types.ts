...
/**
 * Fired when the user executes an interactive `!` / `!!` command.
 * Canonical interception point for platform-default shell execution.
 *
 * `user_bash` remains a legacy compatibility event that is only emitted for
 * Bash execution when no `user_shell` handler handled the command.
 */
export interface UserShellEvent {
	type: "user_shell";
	/** The platform-default shell the command will run under. */
	shell: ShellKind;
	/** The command to execute */
	command: string;
	/** True if !! prefix was used (excluded from LLM context) */
...
/**
 * Result from a user_shell event handler.
 * A handler "handles" the command when it returns either `result` or
 * `operations`; an empty object is treated as unhandled so legacy
 * `user_bash` listeners still get a chance for Bash execution.
 */
export interface UserShellEventResult {
...
export interface ExtensionAPI {
	...
	on(event: "user_shell", handler: ExtensionHandler<UserShellEvent, UserShellEventResult>): void;
