...
export interface Settings {
	...
	pwshPath?: string; // Custom PowerShell 7 executable path (e.g., non-standard install location); supports leading ~ expansion
	quietStartup?: boolean;
	defaultProjectTrust?: DefaultProjectTrust; // default: "ask"; global setting only
	shellCommandPrefix?: string; // Prefix prepended to every bash command (e.g., "shopt -s expand_aliases" for alias support)
	pwshCommandPrefix?: string; // Prefix prepended to every pwsh command (e.g., "$ErrorActionPreference = 'Stop'")
...
export class SettingsManager {
	...
	getPwshPath(): string | undefined {
		const pwshPath = this.settings.pwshPath;
		return pwshPath ? normalizePath(pwshPath) : pwshPath;
	}

	setPwshPath(path: string | undefined): void {
		this.globalSettings.pwshPath = path;
		this.markModified("pwshPath");
		this.save();
	}

	getPwshCommandPrefix(): string | undefined {
		return this.settings.pwshCommandPrefix;
	}

	setPwshCommandPrefix(prefix: string | undefined): void {
		this.globalSettings.pwshCommandPrefix = prefix;
		this.markModified("pwshCommandPrefix");
		this.save();
	}
