...
import { type BashResult, executeShellWithOperations } from "./bash-executor.ts";
import {
...
import type { CustomMessage, ShellExecutionMessage } from "./messages.ts";
...
import type { CommandOperations } from "./tools/command-tool.ts";
import { createAllToolDefinitions, getDefaultToolNames } from "./tools/index.ts";
import { createLocalPwshOperations } from "./tools/pwsh.ts";
...
export interface AgentSessionConfig {
	...
	/** Initial active built-in tool names. Default: read, pwsh, edit, write on Windows; read, bash, edit, write elsewhere */
...
export class AgentSession {
	...
	private _pendingBashMessages: ShellExecutionMessage[] = [];
	...
	/** All messages including custom types like ShellExecutionMessage */
	...
	private _buildRuntime(options: {
		...
		const pwshCommandPrefix = this.settingsManager.getPwshCommandPrefix();
		const pwshPath = this.settingsManager.getPwshPath();
		const baseToolDefinitions = this._baseToolsOverride
			...
			: createAllToolDefinitions(this._cwd, {
					...
					pwsh: { commandPrefix: pwshCommandPrefix, pwshPath },
				});
	...
	/**
	 * Execute an interactive shell command under the given shell.
	 * Adds result to agent context and session.
	 * @param command The shell command to execute
	 * @param shell The shell to execute under (platform default for ! commands)
	 * @param onChunk Optional streaming callback for output
	 * @param options.excludeFromContext If true, command output won't be sent to LLM (!! prefix)
	 * @param options.id Optional identifier included in bash execution update events
	 * @param options.operations Custom CommandOperations for remote execution
	 */
	async executeShell(
		command: string,
		shell: ShellKind,
		onChunk?: (chunk: string) => void,
		options?: { excludeFromContext?: boolean; id?: string; operations?: CommandOperations },
	): Promise<BashResult> {
		const abortController = new AbortController();
		this._bashAbortControllers.add(abortController);

		const prefix =
			shell === "bash" ? this.settingsManager.getShellCommandPrefix() : this.settingsManager.getPwshCommandPrefix();
		const shellPath = shell === "bash" ? this.settingsManager.getShellPath() : this.settingsManager.getPwshPath();
		...
		try {
			const operations =
				...
				(shell === "bash"
					...
					: createLocalPwshOperations({ pwshPath: shellPath }));
			const result = await executeShellWithOperations(
				resolvedCommand,
				shell,
				...
				operations,
				{
					onChunk: (delta) => {
						onChunk?.(delta);
						// RPC streaming events only exist for bash execution; pwsh is
						// interactive-only today and streams through onChunk.
						if (shell === "bash") {
			...
			this.recordShellResult(command, shell, result, options);
			return result;
		} finally {
	...
	async executeBash(
		...
		return this.executeShell(command, "bash", onChunk, options);
	}

	/**
	 * Record a bash execution result in session history.
	 * Used by executeBash and by extensions that handle bash execution themselves.
	 * Kept as the legacy Bash-specific entry point; see recordShellResult.
	 */
	recordBashResult(command: string, result: BashResult, options?: { excludeFromContext?: boolean }): void {
		const bashMessage: ShellExecutionMessage = {
			role: "bashExecution",
			command,
		...
		this._recordShellMessage(bashMessage);
	}

	/**
	 * Record an interactive shell execution result in session history,
	 * preserving the shell that actually executed the command.
	 */
	recordShellResult(
		command: string,
		shell: ShellKind,
		...
		const shellMessage: ShellExecutionMessage = {
			role: "bashExecution",
			shell,
			command,
			output: result.output,
		...
		this._recordShellMessage(shellMessage);
	}

	private _recordShellMessage(shellMessage: ShellExecutionMessage): void {
		// If agent is streaming, defer adding to avoid breaking tool_use/tool_result ordering
		if (this.isStreaming) {
			// Queue for later - will be flushed on agent_end
			this._pendingBashMessages.push(shellMessage);
		} else {
			// Add to agent state immediately
			this.agent.state.messages.push(shellMessage);

			// Save to session
			this.sessionManager.appendMessage(shellMessage);
		}
	}
	...
	/**
	 * Cancel running interactive shell command.
	 */
	abortShell(): void {
	...
	/** Whether an interactive shell command is currently running */
