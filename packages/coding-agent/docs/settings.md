...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `shellPath` | string | - | Custom Bash executable path for the explicit bash tool and Bash-based config-value `!command` resolution (e.g., for Cygwin on Windows); supports a leading `~` for the home directory |
| `shellCommandPrefix` | string | - | Prefix for every bash command (e.g., `"shopt -s expand_aliases"`) |
| `pwshPath` | string | - | Custom PowerShell 7 executable path (e.g., non-standard install location); supports a leading `~` for the home directory |
| `pwshCommandPrefix` | string | - | Prefix for every pwsh command (e.g., `"$ErrorActionPreference = 'Stop'"`) |
...
Interactive `!` / `!!` commands follow the platform-default shell (PowerShell 7 on Windows, Bash on Linux and macOS) and resolve `pwshPath` on Windows. The `shellPath`/`shellCommandPrefix` settings remain Bash-specific and apply to the explicit bash tool and to Bash-based config-value `!command` resolution.
