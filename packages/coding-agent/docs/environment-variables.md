...
- Commands run by the LLM-callable command tools (bash on Linux/macOS, pwsh on Windows) receive `PI_*` variables describing the current session.
...
## Command Tool Session Environment

Commands run by the bash or pwsh tool receive the current Pi session state:
...
The values are resolved when each command starts. Switching models or changing the reasoning level therefore affects the next bash or pwsh command without restarting Pi. `PI_PROVIDER` and `PI_MODEL` identify the selected Pi model, not a different upstream model that a router may choose internally.
...
These variables are injected into the LLM-callable bash and pwsh tools. They are not injected into user-entered `!` or `!!` commands.

### Custom Command Tools

Bash tools created with `createBashTool()` and pwsh tools created with `createPwshTool()` expose the session environment by default when registered with Pi. Injection happens before `spawnHook`, so a hook receives the variables in `ctx.env`:
