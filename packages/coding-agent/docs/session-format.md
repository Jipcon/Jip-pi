...
interface BashExecutionMessage {
  role: "bashExecution";
  shell?: "bash" | "pwsh";       // Shell the command ran under; absent on records written by older versions (implies Bash)
  command: string;
  output: string;
...
The wire role for interactive shell records remains `bashExecution`; no new role was introduced. The optional `shell` field records the actual shell for platform-default interactive `!` execution:

```text
role = "bashExecution", shell absent   → legacy record, interpreted as Bash
role = "bashExecution", shell = "bash" → Bash
role = "bashExecution", shell = "pwsh" → PowerShell 7
```

Forward compatibility of this additive field was verified with the pre-change pi v0.84.1: it loads sessions containing `shell`-tagged `bashExecution` messages and preserves them intact.
