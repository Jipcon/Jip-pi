...
- **Shell command:** `"!command"` at the start executes the whole value as a command and uses stdout (cached for process lifetime). These commands always execute with Bash, regardless of the platform-default shell used by interactive `!` commands.
  ```json
