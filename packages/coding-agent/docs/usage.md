...
Interactive `!` / `!!` commands run in the platform-default shell: PowerShell 7 on Windows, Bash on Linux and macOS. Commands render with a `PS> ` prompt under Pwsh and a `$ ` prompt under Bash.

This platform default applies to the interactive editor commands only. Configuration-value `!command` substitution (for example `apiKey` in `models.json`) still executes Bash; see [models](models.md#value-resolution) and [settings](settings.md#shell).
