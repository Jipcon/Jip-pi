...
```typescript
import { isBashToolResult, isPwshToolResult } from "@earendil-works/pi-coding-agent";
...
pi.on("tool_result", async (event, ctx) => {
  ...
  if (isPwshToolResult(event)) {
    // event.details is typed as PwshToolDetails
  }
...
Fired when the user executes an interactive `!` or `!!` command. This is the **canonical interception point** for interactive shell execution. **Can intercept.**

```typescript
pi.on("user_shell", (event, ctx) => {
  // event.shell - the platform-default shell: "bash" | "pwsh"
  // event.command - the command to execute
  // event.excludeFromContext - true if !! prefix
  // event.cwd - working directory

  // Option 1: Provide custom operations (e.g., SSH, container, VM)
  return { operations: remoteShellOps };
...
**Handled semantics:** a `user_shell` handler only counts as handling the command when it returns `result !== undefined` or `operations !== undefined`. Returning an empty object (`{}`) means "not handled" and lets the dispatcher continue.

**Dispatch order:** `user_shell` runs first. Only when no handler handles it **and** `event.shell === "bash"` does pi fall back to the legacy `user_bash` event. Pwsh commands never consult legacy `user_bash` listeners. The command executes exactly once, by whichever path handled it; handlers can never cause double execution.

For Bash interception, extensions can reuse pi's local shell backend via `createLocalBashOperations()`. The pwsh tool has the symmetric `createLocalPwshOperations()`; both implement the shared `CommandOperations` shape used by the `operations` override.

#### user_bash (legacy compatibility)

Legacy interception event retained for compatibility. Fired only when a `!` / `!!` command resolves to Bash **and** no `user_shell` handler handled it. **Can intercept.**
...
Security-sensitive extensions (containers, sandboxes) should migrate to `user_shell` so Pwsh commands on Windows are intercepted before host execution as well.
...
- `apiKey` - API key literal, environment interpolation (`$ENV_VAR` or `${ENV_VAR}`), or leading `!command` (executed with Bash). Required when defining models (unless `oauth` provided). `$$` escapes `$`, and `$!` escapes a literal `!` without triggering command execution.
...
- [pwsh.ts](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/src/core/tools/pwsh.ts) - `PwshToolDetails`
...
**Operations interfaces:** `ReadOperations`, `WriteOperations`, `EditOperations`, `BashOperations`, `PwshOperations`, `LsOperations`, `GrepOperations`, `FindOperations`

For `user_shell` and `user_bash`, extensions can reuse pi's local shell backend via `createLocalBashOperations()` or `createLocalPwshOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination. Both implement the shared `CommandOperations` shape used by the `operations` override.

The bash and pwsh tools also support a spawn hook to adjust the command, cwd, or env before execution:

```typescript
import { createBashTool, createPwshTool } from "@earendil-works/pi-coding-agent";
...
const pwshTool = createPwshTool(cwd, {
  spawnHook: ({ command, cwd, env }) => ({
    command: `Set-Location /mnt/sandbox${cwd}\n${command}`,
    cwd,
    env: { ...env, CI: "1" },
  }),
});
...
`createBashTool()` and `createPwshTool()` expose the current session to commands through `PI_SESSION_ID`, `PI_SESSION_FILE`, `PI_PROVIDER`, `PI_MODEL`, and `PI_REASONING_LEVEL`. Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
See [Command tool session environment](environment-variables.md#command-tool-session-environment) for variable semantics.
...
| `gondolin/` | Route built-in tools and `!` commands into a Gondolin micro-VM | Tool operations, built-in tool overrides, `on("user_shell")` |
