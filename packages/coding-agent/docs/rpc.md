...
Provider credentials are managed through the credential API; keys and tokens never appear in the protocol. `get_auth_status` returns metadata only, and login flows only carry display events and prompt answers.

#### get_auth_status

List read-only authentication status for every known provider.

```json
{"type": "get_auth_status"}
```

Response `data.providers` is an array of status objects:
...
- `source`: `"stored"`, `"environment"`, `"runtime"`, `"oauth"` or `"none"` (when not configured)
- `mutable`: true when the GUI may store/replace/remove the credential (stored, runtime and oauth credentials are removable; ambient environment credentials are not)
- `supportsApiKey` / `supportsOAuth`: whether the provider accepts api-key / OAuth login
- `oauthName`: selector label for the OAuth login option (when OAuth is available)
- `isSubscription`: true when OAuth access is backed by a provider subscription

#### set_api_key

Store an api key for a provider through the credential API. The key is never echoed back; errors redact it.

```json
{"type": "set_api_key", "provider": "anthropic", "apiKey": "sk-ant-..."}
```

Response: `{"type": "response", "command": "set_api_key", "success": true, "data": {"provider": "anthropic"}}`
...
```json
{"type": "remove_credential", "provider": "anthropic"}
```

Response: `{"type": "response", "command": "remove_credential", "success": true, "data": {"provider": "anthropic"}}`

#### login_oauth

Run a provider's OAuth login flow. Only one login may run at a time. The response is emitted only when the flow completes: `success: true` with `data.provider` when the credential was stored, or `success: false` with an error (e.g. `"Login cancelled"`) when cancelled or failed. While the flow runs, the backend emits `auth_event` and `auth_prompt` records on stdout; the client answers prompts with `auth_prompt_response` on stdin.

```json
{"type": "login_oauth", "provider": "anthropic"}
```

Flow events (stdout):

```json
{"type": "auth_event", "loginId": "uuid", "event": {"type": "auth_url", "url": "https://...", "instructions": "Open this URL"}}
{"type": "auth_event", "loginId": "uuid", "event": {"type": "device_code", "userCode": "ABCD-EFGH", "verificationUri": "https://...", "intervalSeconds": 5, "expiresInSeconds": 600}}
{"type": "auth_event", "loginId": "uuid", "event": {"type": "info", "message": "...", "links": [{"url": "https://...", "label": "Docs"}]}}
{"type": "auth_event", "loginId": "uuid", "event": {"type": "progress", "message": "Waiting for authorization..."}}
```

`event.type` is one of `auth_url`, `device_code`, `info`, `progress`, or `prompt_cancelled`. `prompt_cancelled` carries a `requestId` and means an earlier `auth_prompt` was aborted out-of-band (for example a callback server won the race against a `manual_code` prompt); the client should clear that prompt's input state.

Prompts (stdout) mirror the runtime prompt types without the runtime-only signal:

```json
{"type": "auth_prompt", "loginId": "uuid", "requestId": "prompt-uuid", "prompt": {"type": "manual_code", "message": "Paste the authorization code"}}
```

`prompt.type` is one of `text`, `secret`, `select` (with `options` of `{id, label, description?}`), or `manual_code`.

Prompt answers (stdin):

```json
{"type": "auth_prompt_response", "requestId": "prompt-uuid", "value": "ABCDEFGH"}
{"type": "auth_prompt_response", "requestId": "prompt-uuid", "cancelled": true}
```

`auth_prompt_response` is handled out-of-band and does not produce a response record. Answers are redacted from any login error message.

Opening the authorization URL in a browser is the client's responsibility (`auth_url` is just an event). The localhost callback server, when the provider uses one, runs inside the backend process.

#### cancel_login_oauth

Cancel the in-flight `login_oauth` flow (idempotent). The pending `login_oauth` command then responds with `success: false` and error `"Login cancelled"`.

```json
{"type": "cancel_login_oauth"}
```

Response: `{"type": "response", "command": "cancel_login_oauth", "success": true}`

#### refresh_auth

Re-read shared credentials and refresh the model catalog. Used to synchronize pooled backends after a credential change elsewhere (credentials live in a shared auth file; the catalog needs an explicit refresh).

```json
{"type": "refresh_auth"}
{"type": "refresh_auth", "provider": "anthropic"}
```

Response: `{"type": "response", "command": "refresh_auth", "success": true}`
...
Execute a Bash command and add output to conversation context. The `bash` command always executes with Bash, regardless of the platform-default shell used by interactive `!` commands. Output streams as `bash_execution_update` events while the command runs; the response contains the final result.
...
The `bash` command executes immediately and returns a `BashResult`. Internally, a `BashExecutionMessage` is created and stored in the agent's message state with `shell: "bash"`.
...
Interactive `!` execution persists the same role with `shell` set to the platform-default shell (`"bash"` or `"pwsh"`); records without the field are legacy Bash records.
...
For a complete example of handling the extension UI protocol, see [`examples/rpc-extension-ui.ts`](../examples/rpc-extension-ui.ts).
