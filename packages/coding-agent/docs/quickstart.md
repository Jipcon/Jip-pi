...
The command output is sent to the model. Use `!!command` to run a command without adding its output to the model context. Interactive `!` commands use the platform-default shell: PowerShell 7 on Windows, Bash on Linux and macOS (rendered as `PS> command` and `$ command`, respectively).
