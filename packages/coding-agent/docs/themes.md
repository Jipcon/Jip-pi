...
| Token | Purpose |
|-------|---------|
| `shellMode` | Editor border in interactive shell mode (`!` prefix). Optional; falls back to `bashMode` when absent |
| `bashMode` | Legacy compatibility token. Still honored when `shellMode` is not set |

`shellMode` wins when both tokens are present. Existing themes that only define `bashMode` keep working unchanged.
