...
- Default built-ins: `read`, `pwsh`, `edit`, `write` on Windows; `read`, `bash`, `edit`, `write` elsewhere
- `noTools: "all"` disables all tools
- `noTools: "builtin"` disables default built-ins while keeping extension and custom tools enabled
- `excludeTools` disables specific built-in, extension, or custom tool names after any `tools` allowlist is applied
- An explicit `tools` list is never rewritten by platform defaults: `tools: ["read", "bash"]` enables exactly `read` and `bash`, even on Windows
...
createCodingTools
createReadOnlyTools
createReadTool, createBashTool, createPwshTool, createEditTool, createWriteTool
