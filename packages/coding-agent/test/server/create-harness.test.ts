...
describe("coding-agent Harness construction", () => {
	...
	test("builds the system prompt from logicalCwd while tools keep the physical env cwd", async () => {
		...
		const session = new Session(new InMemorySessionStorage({ id: "logical-cwd-session", createdAt: 1 }));
		const physicalCwd = "/physical/lease/candidate-1";
		const env = new NodeExecutionEnv({ cwd: physicalCwd });
		try {
			const created = await createCodingAgentHarness({
				session,
				models: createModels(),
			...
			try {
				const prompt = await resolveSystemPrompt(configuredSystemPrompt);
				expect(prompt).toContain("/workspace");
				expect(prompt).not.toContain("/physical/lease/candidate-1");
				// Tools still execute against the physical env cwd.
				const write = (await created.harness.getTools()).find((tool) => tool.name === "write");
