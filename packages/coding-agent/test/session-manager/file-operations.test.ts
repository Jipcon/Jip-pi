...
describe("SessionManager explicit empty-session persistence", () => {
	let tempDir: string;
	...
	it("persists a header-only session only when explicitly requested", () => {
		const session = SessionManager.create(tempDir, tempDir);
		const sessionFile = session.getSessionFile();
		if (!sessionFile) {
			throw new Error("Expected a persistent session file path");
		}

		expect(existsSync(sessionFile)).toBe(false);
		expect(session.persistSession()).toBe(sessionFile);
		expect(existsSync(sessionFile)).toBe(true);
		expect(loadEntriesFromFile(sessionFile)).toEqual([
			expect.objectContaining({ type: "session", id: session.getSessionId(), cwd: tempDir }),
		]);
	});
});
