...
describe("AgentSessionRuntime characterization", () => {
	...
	it("preserves an explicitly configured session directory when resuming an external session", async () => {
		const tempDir = join(tmpdir(), `pi-runtime-session-dir-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		const configuredSessionDir = join(tempDir, "configured-sessions");
		const externalSessionDir = join(tempDir, "external-sessions");
		const { runtime } = await createRuntimeForTest(() => {}, {
			cwd: tempDir,
			sessionDir: configuredSessionDir,
		});
		const externalSession = SessionManager.create(tempDir, externalSessionDir);
		externalSession.appendMessage({
			role: "user",
			content: [{ type: "text", text: "external" }],
			timestamp: Date.now(),
		});
		externalSession.appendMessage(fauxAssistantMessage("external response"));
		const externalSessionFile = externalSession.getSessionFile();
		expect(externalSessionFile).toBeDefined();

		await runtime.switchSession(externalSessionFile!);

		expect(runtime.session.sessionManager.getSessionDir()).toBe(configuredSessionDir);
		expect(runtime.session.sessionManager.usesDefaultSessionDir()).toBe(false);

		await runtime.newSession();
		expect(runtime.session.sessionFile?.startsWith(configuredSessionDir)).toBe(true);
	});
