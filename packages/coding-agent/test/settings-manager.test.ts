...
describe("SettingsManager", () => {
	...
	describe("getPwshPath", () => {
		it("should return undefined when not set", () => {
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ theme: "dark" }));
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.getPwshPath()).toBeUndefined();
		});

		it("should return an absolute pwshPath unchanged", () => {
			writeFileSync(
				join(agentDir, "settings.json"),
				JSON.stringify({ pwshPath: "C:\\Program Files\\PowerShell\\7\\pwsh.exe" }),
			);
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.getPwshPath()).toBe("C:\\Program Files\\PowerShell\\7\\pwsh.exe");
		});

		it("should expand ~ in pwshPath", () => {
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ pwshPath: "~/.local/bin/pwsh" }));
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.getPwshPath()).toBe(join(homedir(), ".local/bin/pwsh"));
		});

		it("should preserve pwshPath when saving unrelated settings", async () => {
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ pwshPath: "~/.local/bin/pwsh" }));
			const manager = SettingsManager.create(projectDir, agentDir);
			manager.setTheme("light");
			await manager.flush();

			const savedSettings = JSON.parse(readFileSync(settingsPath, "utf-8"));
			expect(savedSettings.pwshPath).toBe("~/.local/bin/pwsh");
			expect(savedSettings.theme).toBe("light");
		});
	});

	describe("getPwshCommandPrefix", () => {
		it("should return undefined when not set", () => {
			writeFileSync(join(agentDir, "settings.json"), JSON.stringify({ theme: "dark" }));
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.getPwshCommandPrefix()).toBeUndefined();
		});

		it("should return the configured prefix", () => {
			writeFileSync(
				join(agentDir, "settings.json"),
				JSON.stringify({ pwshCommandPrefix: "$ErrorActionPreference = 'Stop'" }),
			);
			const manager = SettingsManager.create(projectDir, agentDir);
			expect(manager.getPwshCommandPrefix()).toBe("$ErrorActionPreference = 'Stop'");
		});

		it("should preserve pwshCommandPrefix when saving unrelated settings", async () => {
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ pwshCommandPrefix: "$ErrorActionPreference = 'Stop'" }));
			const manager = SettingsManager.create(projectDir, agentDir);
			manager.setTheme("light");
			await manager.flush();

			const savedSettings = JSON.parse(readFileSync(settingsPath, "utf-8"));
			expect(savedSettings.pwshCommandPrefix).toBe("$ErrorActionPreference = 'Stop'");
			expect(savedSettings.theme).toBe("light");
		});
	});
});
