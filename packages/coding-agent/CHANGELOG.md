...
- On Windows, interactive `!` / `!!` commands now execute with PowerShell 7 instead of Bash. The explicit `bash` tool and the `bash` RPC command remain Bash on every platform.

### Added

- Added the `user_shell` extension event as the canonical interception point for interactive `!` / `!!` commands, carrying the resolved shell (`bash` | `pwsh`), with legacy `user_bash` fallback for unhandled Bash commands. Handlers are considered to handle a command only when they return `result` or `operations`.
- Added the optional `shell` field to persisted `bashExecution` messages and the `shellMode` theme token with `bashMode` fallback.

- Added `login_oauth`, `cancel_login_oauth`, `auth_prompt_response`, and `refresh_auth` RPC commands with `auth_event`/`auth_prompt` flow events, so GUI hosts can run provider OAuth login flows (authorization URLs, device codes, prompts) without ever seeing credentials.
- Added `supportsOAuth`, `oauthName`, and `isSubscription` fields to RPC `get_auth_status` provider entries.
- Added `list_sessions`, `rename_session`, and `persist_session` RPC commands for enumerating, renaming, and explicitly saving empty sessions for GUI hosts.
- Added a first-class built-in `pwsh` tool (PowerShell 7) as the platform-default command tool on Windows, with `pwshPath`/`pwshCommandPrefix` settings, `createPwshTool()`, `createPwshToolDefinition()`, `createLocalPwshOperations()`, `PwshOperations`, and `isPwshToolResult()`. Default built-in tools are now `read`, `pwsh`, `edit`, `write` on Windows and `read`, `bash`, `edit`, `write` elsewhere; explicit `--tools` selections are never rewritten.
- Exported `findInitialModel` (with `InitialModelResult`) and `DEFAULT_THINKING_LEVEL` from the SDK entry point so SDK hosts can predict the model and thinking level a fresh session will materialize with.

### Fixed

- Fixed RPC `get_auth_status` reporting stored OAuth credentials as non-removable (`mutable: false`) when the provider has no api-key login.
- Fixed resuming a session outside an explicitly configured session directory replacing that storage root and hiding its sessions from subsequent RPC listings.
