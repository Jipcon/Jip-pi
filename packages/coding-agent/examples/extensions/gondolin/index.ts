...
/**
 * PowerShell 7 operations routing interactive ! commands into the guest VM.
 * Requires pwsh inside the guest (probed at VM start).
 */
function createGondolinPwshOps(vm: VM, localCwd: string, pwshPath: string): BashOperations {
	return {
		exec: async (command, cwd, { onData, signal }) => {
			if (signal?.aborted) throw new Error("aborted");
			const proc = vm.exec([pwshPath, "-NoLogo", "-NoProfile", "-NonInteractive", "-Command", command], {
...
export default function (pi: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const pwshProbe = await created.exec(["/bin/sh", "-lc", "command -v pwsh || true"]);
		pwshPath = pwshProbe.stdout.trim() || undefined;
		vm = created;
		ctx?.ui.setStatus(
			"gondolin",
	...
	pi.on("user_shell", async (event, ctx) => {
		const activeVm = await ensureVm(ctx);
		if (event.shell === "bash") {
			return { operations: createGondolinBashOps(activeVm, localCwd, shellPath) };
		}
		if (!pwshPath) {
			// Fail closed: never let a Pwsh command fall through to the host.
			return {
				result: {
					output: "PowerShell 7 is not available in the Gondolin VM; command not executed on the host.",
					exitCode: 127,
					cancelled: false,
					truncated: false,
				},
		...
		return { operations: createGondolinPwshOps(activeVm, localCwd, pwshPath) };
	});

	// Legacy interception path for older pi versions that only emit user_bash.
