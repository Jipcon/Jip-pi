/**
 * Shared three-phase tool execution primitives:
 * prepare (lookup + argument preparation + schema validation + before hook),
 * execute (effect with update events), finalize (after hook + normalization).
 * Extracted from the legacy agent loop so Harness v4 can insert durable
 * transactions between phases without changing the low-level behavior.
 */
