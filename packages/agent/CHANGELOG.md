...
- Added `ShellKind` and `getDefaultShellKind()` as the shared platform-default shell resolver export from `@earendil-works/pi-agent-core` (`win32` → `pwsh`, otherwise → `bash`).

- Added the behavior-free `harness-v4` contract surface for entries, registers, usage, operation state, storage, sessions, runtime effects, and public harness APIs.
- Added the Harness v4 session foundation with runtime codecs, UUIDv7 ids, context projection, atomic in-memory storage, repository lifecycle and forks, backend conformance tests, and instrumented storage.
- Added the Harness v4 R1 runtime shell with five-register restore and bounded hydration, serialized lane/settings mutation lines, register-sequence transition guards, runtime snapshots, manual effect gating, hooks/events, and fault/close plumbing.
